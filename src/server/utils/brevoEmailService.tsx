...
interface EmailPayload {
  to: { email: string }[];
  sender: { email: string | undefined; name: string | undefined };
  subject: string;
  htmlContent: string;
  textContent: string;
  tags: string[];
 ...
 * @param {EmailData} emailData - Email configuration
 * @returns {Promise<BrevoResponse>} Response from Brevo API
 */
export const sendEmailViaBrevo = async (emailData: EmailData): Promise<BrevoResponse> => {
  try {
    ...
    const emailPayload: EmailPayload = {
      ...
      tags: (tags && tags.length > 0) ? tags : ['general'], // ← Brevo requires at least one tag
    };
 ...
 * @param {EmailData[]} emailList - Array of email data objects
 * @returns {Promise<BulkEmailResults>} Results of bulk send
 */
export const sendBulkEmailsViaBrevo = async (emailList: EmailData[]): Promise<BulkEmailResults> => {
  try {
    if (!brevoApiKey) {
      throw new Error('BREVO_API_KEY is not configured');
    }
      ...
      try {
        ...
        if (result.success) {
          results.successful.push({
            email: typeof emailData.to === 'string' ? emailData.to : emailData.to[0],
 ...
 * @param {ContactData} contactData - Contact information
 * @returns {Promise<BrevoResponse>} Response from Brevo API
 */
export const createBrevoContact = async (contactData: ContactData): Promise<BrevoResponse> => {
  try {
    if (!brevoApiKey) {
      throw new Error('BREVO_API_KEY is not configured');
    }
 ...
 * @param {UpdateContactData} updateData - Updated contact information
 * @returns {Promise<BrevoResponse>} Response from Brevo API
 */
export const updateBrevoContact = async (email: string, updateData: UpdateContactData): Promise<BrevoResponse> => {
  try {
    if (!brevoApiKey) {
      throw new Error('BREVO_API_KEY is not configured');
    }
 ...
 * @param {Contact[]} contacts - Contacts to add
 * @returns {Promise<BrevoResponse>} Response from Brevo API
 */
export const addContactsToList = async (listId: number, contacts: Contact[]): Promise<BrevoResponse> => {
  try {
    if (!brevoApiKey) {
      throw new Error('BREVO_API_KEY is not configured');
    }
    ...
    const payload = {
      emails: contacts.map((c: Contact) => c.email),
    };
 ...
 * @returns {Promise<BrevoResponse>} Response from Brevo API
 */
export const removeContactsFromList = async (listId: number, emails: string[]): Promise<BrevoResponse> => {
  try {
    if (!brevoApiKey) {
      throw new Error('BREVO_API_KEY is not configured');
    }
 ...
 * @param {string} email - Contact email
 * @returns {Promise<BrevoResponse>} Contact information
 */
export const getBrevoContact = async (email: string): Promise<BrevoResponse> => {
  try {
    if (!brevoApiKey) {
      throw new Error('BREVO_API_KEY is not configured');
    }
 ...
 * @returns {Promise<BrevoResponse>} Response from Brevo API
 */
export const deleteBrevoContact = async (email: string): Promise<BrevoResponse> => {
  try {
    if (!brevoApiKey) {
      throw new Error('BREVO_API_KEY is not configured');
    }
    ...
    if (response.status === 204) {
      return { success: true, message: 'Contact deleted successfully' } as BrevoResponse;
    }
 ...
 * @param {EmailFilters} filters - Filter parameters
 * @returns {Promise<BrevoResponse>} Email events
 */
export const getEmailEvents = async (filters: EmailFilters = {}): Promise<BrevoResponse> => {
  try {
    if (!brevoApiKey) {
      throw new Error('BREVO_API_KEY is not configured');
    }
 ...
 * @returns {Promise<BrevoResponse>} Parsed response
 */
async function handleBrevoResponse(response: Response): Promise<BrevoResponse> {
  try {
 ...
 */
export const verifyBrevoApiKey = async (): Promise<boolean> => {
  try {
    if (!brevoApiKey) {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    console.error('Error verifying Brevo API Key:', errorMessage);
    return false;
  }
};
