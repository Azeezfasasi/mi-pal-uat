...
export default function EditProject() {
  ...
  const [existingLogoImage, setExistingLogoImage] = useState<ExistingImage | null>(null);
  const [existingThumbnailImage, setExistingThumbnailImage] = useState<ExistingImage | null>(null);
  const [existingSlides, setExistingSlides] = useState<ExistingImage[]>([]);
    ...
    const fetchProject = async () => {
      try {
        ...
        if (project.logoImage) {
          setExistingLogoImage(project.logoImage);
        }
        if (project.thumbnailImage) {
          setExistingThumbnailImage(project.thumbnailImage);
        }
        if (project.slides && project.slides.length > 0) {
          setExistingSlides(project.slides);
        }
  ...
  const handleImageChange = (file: File | null, type: 'logo' | 'thumbnail') => {
    ...
    reader.onloadend = () => {
      ...
      if (type === 'logo') {
        ...
        setExistingThumbnailImage(null);
        setFormData(prev => ({
          ...prev,
          thumbnailImage: file,
        }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (existingSlides.length === 0 && formData.slides.length === 0) {
    ...
    try {
      const projectData = {
        ...
        logoImage: logoPreview ? { url: logoPreview.preview, publicId: '' } : existingLogoImage,
        thumbnailImage: thumbnailPreview ? { url: thumbnailPreview.preview, publicId: '' } : existingThumbnailImage,
        slides: [
          ...existingSlides,
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8 flex items-center gap-4">
          <button
            onClick={() => router.back()}
            className="inline-flex items-center justify-center rounded-lg p-2 text-gray-700 hover:bg-gray-200 transition"
          >
            <ArrowLeft size={24} />
          </button>
          <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-lg shadow-sm p-6 sm:p-8">
          ...
          <div>
            ...
            {existingLogoImage && !logoPreview && (
              <div className="mb-4 relative h-32 w-full bg-gray-100 rounded-lg p-4">
                <Image
                  src={existingLogoImage.url}
                  alt="Existing logo"
                  fill
          ...
          <div>
            ...
            {existingThumbnailImage && !thumbnailPreview && (
              <div className="mb-4 relative h-40 w-full bg-gray-100 rounded-lg p-4">
                <Image
                  src={existingThumbnailImage.url}
          ...
          <div>
            ...
            {existingSlides.length > 0 && (
              <div className="mb-4">
                <h4 className="text-sm font-medium text-gray-900 mb-3">
                  Current Slides ({existingSlides.length})
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  {existingSlides.map((slide, index) => (
                    <div key={index} className="relative group">
                      ...
                      <button
                        type="button"
                        onClick={() => removeExistingSlide(index)}
            ...
            >
              <div className="text-center">
                ...
                <p className="mt-2 text-sm text-gray-600">Click to add more slide images</p>
            ...
            {slidePreviews.length > 0 && (
              <div className="mt-4">
                <h4 className="text-sm font-medium text-gray-900 mb-3">
                  New Slides ({slidePreviews.length})
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
